...
minc_lms = clamp.(quavectors(th.-0.5*ConeIsoRGBVec),0,1);maxc_lms = clamp.(quavectors(th.+0.5*ConeIsoRGBVec),0,1)
...
mmaxc_lms = clamp.(LMSToRGB*ContrastToLMS*quavectors(ccmf'.*trivectors(maxcc)),0,1)
mminc_lms = clamp.(LMSToRGB*ContrastToLMS*quavectors(ccmf'.*trivectors(mincc)),0,1)
...
# since through color is the center of RGB cube, the line intersects at two symmatric points on the faces of unit cube
minc_dkl = clamp.(quavectors(th.-0.5*DKLIsoRGBVec),0,1);maxc_dkl = clamp.(quavectors(th.+0.5*DKLIsoRGBVec),0,1)
...
hslhuewp[3,:] .= 0.2
huewp_hsl = clamp.(XYZToRGB*quavectors(xyY2XYZ(hslhuewp)),0,1)
